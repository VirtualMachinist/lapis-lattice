...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    /// Lapis patch: enable or disable caret blinking (enabled by default).
    ///
    /// Disabled keeps a steady caret and stops the 500 ms blink timer, so a focused
    /// input no longer notifies its observers (and redraws its window) while idle.
    /// Focus, window activation and keystrokes do not restart it while disabled.
    pub fn set_cursor_blink(&mut self, enabled: bool, cx: &mut Context<Self>) {
        self.blink_cursor.update(cx, |cursor, cx| cursor.set_enabled(enabled, cx));
    }
